...
-- Count of no.of.rows in each table.

SELECT COUNT(*) AS No_of_rows_count FROM MOVIE
UNION 
...
# we able to find the total number of rows in each table  of the schema 
-- director_mapping = 3867 rows, genre = 14662 rows, movie = 7672 rows, names = 26619 rows, ratings = 8230 rows, role_mapping = 14315 rows
...
SELECT Sum(CASE
       ...
       Sum(CASE
             ...
             ELSE 0
           END) AS Yr_NULL_COUNT,
       Sum(CASE
             WHEN date_published IS NULL THEN 1
             ELSE 0
           END) AS dt_pub_NULL_COUNT,
       Sum(CASE
             WHEN duration IS NULL THEN 1
             ELSE 0
           END) AS dur_NULL_COUNT,
       Sum(CASE
...
-- Number of movies released each month --> 2017	3052 , 2018	2944, 2019	2001
...
-- Drama type genre movies were published more(4285) in the year 2017.
...
# totally we have 3289 movies with only one genre
...
-- Displaying the top 10 movies using RANK() function and LIMIT clause
SELECT     title,
           avg_rating,
           dense_rank() 
             OVER(
...
# Dream Warrior Pictures , National Theatre Live are the production company has produce the most number of hit movies (average rating > 8)
...
(
 ...
 (
	 ...
	 FROM
		 ...
		 JOIN movie AS m 
         ON m.id=r.movie_id
		 JOIN avg_inter_movie_days AS a 
